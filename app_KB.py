...
st.markdown(
    """<h1 style='text-align: center;'> Breast Cancer Prediction Dataset </h1> """,
...
if selected == "Beranda":
    ...
    st.write(
        """breast cancer adalah suatu kanker dimana bertumbuhnya serta berkembangnya sebuah sel-sel jaringan yang mengerikan 
    ...
    st.write(
        """Secara umum, semakin tinggi tingkat keparahan kanker (semakin berkembang dan menyebar), 
...
5. Riwayat persalinan dan menstruasi. Semakin tua seorang wanita saat melahirkan anak pertamanya, semakin besar risikonya terkena kanker payudara. Juga berisiko lebih tinggi adalah:
 ...
 """
    ...
    st.subheader(""" Tujuan""")
    st.write(
        """ Analisis ini bertujuan untuk mengamati fitur mana yang paling membantu dalam memprediksi 
    ...
    <a href="https://www.kaggle.com/datasets/merishnasuwal/breast-cancer-prediction-dataset"> kaggle dataset</a>""",
        unsafe_allow_html=True,
    )
...
if selected == "Dataset":
    st.markdown(
        """<h2 style='text-align: center; color:grey;'> Dataset Breast Cancer Prediction </h1> """,
        unsafe_allow_html=True,
    )
    df = pd.read_csv(
        "https://raw.githubusercontent.com/aisyaturradiah/project/main/Breast_cancer_data.csv"
    )
...
if selected == "Preprocessing":
    ...
    labels = pd.DataFrame({"jinak": [dumies[0]], "ganas": [dumies[1]]})
...
if selected == "Modelling":
    ...
    training, test = train_test_split(
        scaled_features, test_size=0.4, random_state=0
    ...
    if submitted:
        if naive:
            st.write(
                "Model Naive Bayes accuracy score: {0:0.2f}".format(gaussian_akurasi)
            )
...
if selected == "Referensi":
    ...
    st.markdown(
        "<h1 style='font-size:15px;text-align: left; color: black;'>Kelompok 11</h1>",
        unsafe_allow_html=True,
    )
    st.markdown("1. Kharisma Intan Safitri (200411100010)", unsafe_allow_html=True)
    st.markdown("2. Aisyah Meta Sari Putri (200411100031)", unsafe_allow_html=True)
    ...
    st.subheader("""""")
    st.write(
        """Sumber data di dapatkan melalui github, Berikut merupakan link untuk mengakses sumber dataset dari github.
    <a href="https://raw.githubusercontent.com/aisyaturradiah/project/main/Breast_cancer_data.csv">github dataset dataset</a>""",
        unsafe_allow_html=True,
    )

    st.subheader("""Sumber Jurnal""")
    st.write(
        """Sumber informasi terdapat pada jurnal terkait, Berikut merupakan link untuk mengakses jurnalnya.
    <a href="https://drive.google.com/drive/folders/1btF7Kj5EKYtaoKTWcMMkyTekdhXd7j4N?usp=sharing">jurnal reference</a>""",
        unsafe_allow_html=True,
    )

    st.subheader(""" """)
    st.write(
        """ Sumber referensi dari deskripsi setiap fitur yang terdapat dalam dataset breast cancer, berikut merupakan link untuk mengakses informasi dari website.
    <a href="https://medium.com/swlh/simple-machine-learning-model-on-breast-cancer-dataset-c1a013b594ad">fitur deskription</a>""",
        unsafe_allow_html=True,
    )
